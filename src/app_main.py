...
class ETLManagerWindow(QMainWindow):
    ...
    def __init__(
        self, startup_progress_callback: Optional[Callable[[int, str], None]] = None
    ):
        super().__init__()
    ...
    def _load_etl_tabs(self):
        ...
        for idx, etl_info in enumerate(etls, start=1):
            try:
                container, placeholder = self._create_lazy_tab_container(
                    etl_info["name"]
                )
                ...
                self._report_startup_progress(
                    progress, f"Registrando ETL: {etl_info['name']}"
                )
    ...
    def _ensure_tab_loaded(self, index: int):
        ...
        try:
            ...
            self._set_lazy_message(
                tab_info, f"No se pudo cargar {tab_info['name']}.\n{e}", is_error=True
            )
