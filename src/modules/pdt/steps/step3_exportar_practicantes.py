...
def seleccionar_y_convertir_columnas(df: pl.DataFrame, esquema: dict) -> pl.DataFrame:
    ...
    print(
        f"  ✓ Resultado: {df_resultado.height:,} filas × {df_resultado.width} columnas"
    )
...
def aplicar_business_rules(df: pl.DataFrame) -> pl.DataFrame:
    ...
    print("  ✓ Universidad de Procedencia:")
...
def agregar_columna_enriquecida(df: pl.DataFrame) -> pl.DataFrame:
    ...
    print("  ✓ Columna NOMBRE_MES agregada exitosamente")

    print("  ✓ Columnas reordenadas (NOMBRE_MES después de MES)")
...
def generar_metricas_basicas(df: pl.DataFrame):
    ...
    if "Universidad de Procedencia" in df.columns:
        universidades = (
            df["Universidad de Procedencia"]
            .value_counts()
            .sort("Universidad de Procedencia")
        )
        print("\nUniversidades de Procedencia:")
        for row in universidades.iter_rows(named=True):
            print(
                f"   {row['Universidad de Procedencia']:40}: {row['count']:4} practicantes"
            )
...
def guardar_resultados(
    df: pl.DataFrame, carpeta_silver: Path, export_excel: bool = False
):
    """
    ...
    nombre_actual = "ingresos_practicantes_gold"
    ...
    nombre_historico = f"ingresos_practicantes_gold_{timestamp}"
...
def main():
    ...
    try:
        ...
        print(
            f"  ✓ Datos cargados: {df_silver.height:,} filas × {df_silver.width} columnas"
        )
