...
import sys
import time
from src.utils.validate_source import (
...
from src.utils.lazy_loader import create_etl_loader  # noqa: E402
...
class PDTWorker(BaseETLWorker):
    ...
    def __init__(self, archivos, output_dir, export_excel_gold: bool = False):
        ...
        # Timers
        self.timers = {"total": 0, "step1": 0, "step2": 0, "step3": 0}
    ...
    def execute_etl(self) -> Dict:
        ...
        try:
            ...
            try:
                ...
                self.logger.info("  📄 Hojas esperadas: EMPLEADOS, PRACTICANTES")
                ...
                for idx, (nombre_hoja, config) in enumerate(
                    CONFIGURACION_HOJAS.items(), 1
                ):
                    ...
                    self.logger.info("")
                    self.logger.info(
                        f"[{idx}/{total_hojas}] PROCESANDO HOJA: {nombre_hoja}"
                    )
                    self.logger.info("-" * 70)

                    try:
                        # Leer datos
                        df_original = leer_hoja_excel(
                            archivo_bronze, nombre_hoja, config
                        )

                        if df_original.is_empty():
                            self.logger.warning(
                                f"⚠️ No se encontraron datos en {nombre_hoja}"
                            )
                            continue
                        ...
                        self.logger.info(
                            f"✓ Hoja {nombre_hoja} procesada: {df_limpio.height:,} registros"
                        )
                ...
                ruta_empleados_silver = rutas_guardadas.get("EMPLEADOS", {}).get(
                    "parquet"
                )
                ruta_practicantes_silver = rutas_guardadas.get("PRACTICANTES", {}).get(
                    "parquet"
                )
                ...
                self.logger.info(
                    f"  ⏱️ Duración: {self.logger.format_duration(self.timers['step1'])}"
                )
                self.logger.info("-" * 70)
            ...
            self.progress_updated.emit(
                50, "🔄 Iniciando transformación Gold - EMPLEADOS..."
            )
            ...
            try:
                ...
                esquema_path = resolve_structured_path(
                    "assets/esquemas/esquema_relacion_ingresos"
                )
                ...
                if not esquema_path.exists():
                    raise FileNotFoundError(
                        f"No se encontró esquema en: {esquema_path}"
                    )

                self.progress_updated.emit(55, "📋 Cargando esquema EMPLEADOS...")

                esquema_completo = load_structured_data(
                    esquema_path, prefer_resource_path=False
                )

                if (
                    "hojas" not in esquema_completo
                    or "EMPLEADOS" not in esquema_completo["hojas"]
                ):
                ...
                self.logger.info(
                    f"✓ Esquema cargado: v{esquema_completo['metadata']['version']}"
                )
                self.logger.info(
                    f"  • Columnas esperadas para EMPLEADOS: {len(esquema['schema'])}"
                )

                self.progress_updated.emit(
                    60, "📊 Cargando datos Silver - EMPLEADOS..."
                )

                # Verificar que tenemos la ruta de EMPLEADOS
                if not ruta_empleados_silver or not ruta_empleados_silver.exists():
                    raise FileNotFoundError(
                        "No se encontró el archivo Silver de EMPLEADOS"
                    )
                ...
                self.logger.info(
                    f"✓ Datos silver cargados: {len(df_silver):,} registros"
                )

                self.progress_updated.emit(65, "⚙️ Cargando módulo de transformación...")

                # LAZY LOADING: step2 se carga AQUÍ
                seleccionar_y_convertir_columnas = (
                    self.loader.step2.seleccionar_y_convertir_columnas
                )
                ...
                self.logger.info("✓ Transformaciones aplicadas")
                self.logger.info("  • Columna NOMBRE_MES agregada después de MES")
                self.logger.info(f"  • Registros finales: {len(df_gold):,}")
                self.logger.info(f"  • Columnas finales: {len(df_gold.columns)}")

                self.progress_updated.emit(
                    78, "💾 Guardando archivos Gold - EMPLEADOS..."
                )
                ...
                (
                    ruta_parquet_actual,
                    ruta_excel_actual,
                    ruta_parquet_historico,
                    ruta_excel_historico,
                ) = guardar_resultados_gold(
                    df_gold,
                    carpeta_silver,
                ...
                self.logger.info("-" * 70)
                self.logger.info("✓ Step 2 completado exitosamente")
                ...
                self.logger.info(
                    f"  ⏱️ Duración: {self.logger.format_duration(self.timers['step2'])}"
                )
                self.logger.info("-" * 70)

                self.progress_updated.emit(
                    80, f"✓ EMPLEADOS Gold generado: {len(df_gold):,} registros"
                )
            ...
            self.logger.info("=" * 70)
            self.logger.info(
                "STEP 3: TRANSFORMACIÓN GOLD (Silver → Gold) - PRACTICANTES"
            )
            self.logger.info("=" * 70)

            self.progress_updated.emit(
                82, "🔄 Iniciando transformación Gold - PRACTICANTES..."
            )
            ...
            try:
                # Buscar y cargar esquema
                esquema_path = resolve_structured_path(
                    "assets/esquemas/esquema_ingresos_practicantes"
                )
                ...
                if (
                    "hojas" not in esquema_completo
                    or "PRACTICANTES" not in esquema_completo["hojas"]
                ):
                    raise ValueError(
                        "Esquema no contiene configuración para PRACTICANTES"
                    )
                ...
                self.logger.info(
                    f"  • Columnas esperadas para PRACTICANTES: {len(esquema['schema'])}"
                )

                self.progress_updated.emit(
                    86, "📊 Cargando datos Silver - PRACTICANTES..."
                )

                # Verificar que tenemos la ruta de PRACTICANTES
                if (
                    not ruta_practicantes_silver
                    or not ruta_practicantes_silver.exists()
                ):
                    raise FileNotFoundError(
                        "No se encontró el archivo Silver de PRACTICANTES"
                    )
                ...
                seleccionar_y_convertir_columnas_prac = (
                    self.loader.step3.seleccionar_y_convertir_columnas
                )
                ...
                self.logger.info("✓ Transformaciones aplicadas")
                self.logger.info(
                    "  • Business rules aplicadas (Universidad de Procedencia)"
                )
                self.logger.info("  • Columna NOMBRE_MES agregada después de MES")
                self.logger.info(f"  • Registros finales: {len(df_gold):,}")
                self.logger.info(f"  • Columnas finales: {len(df_gold.columns)}")
                ...
                self.progress_updated.emit(
                    95, "💾 Guardando archivos Gold - PRACTICANTES..."
                )
                ...
                self.logger.info("-" * 70)
                self.logger.info("✓ Step 3 completado exitosamente")
                self.logger.info(f"  • Registros Gold: {len(df_gold):,}")
                self.logger.info(f"  • Columnas Gold: {len(df_gold.columns)}")
                self.logger.info(f"  • Parquet (actual): {ruta_parquet_actual.name}")
                if ruta_excel_actual is not None:
                    self.logger.info(f"  • Excel (actual): {ruta_excel_actual.name}")
                self.logger.info(
                    f"  ⏱️ Duración: {self.logger.format_duration(self.timers['step3'])}"
                )
                self.logger.info("-" * 70)

                self.progress_updated.emit(
                    98, f"✓ PRACTICANTES Gold generado: {len(df_gold):,} registros"
                )
            ...
            if "step3" in resultado and "registros" in resultado["step3"]:
                lineas_mensaje.append(
                    f"  • PRACTICANTES (Gold): {resultado['step3']['registros']:,} registros"
                )
            ...
            if self.timers["step2"] > 0:
                lineas_mensaje.append(
                    f"    - Step 2 (EMPLEADOS→Gold): {self.logger.format_duration(self.timers['step2'])}"
                )

            if self.timers["step3"] > 0:
                lineas_mensaje.append(
                    f"    - Step 3 (PRACTICANTES→Gold): {self.logger.format_duration(self.timers['step3'])}"
                )
            ...
            return self.build_error_result(
                stage_name="ETL completo PDT", error=str(e), timers=self.timers
            )
