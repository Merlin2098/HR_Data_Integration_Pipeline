...
def cargar_query_sql() -> str:
    ...
    if not ruta_query.exists():
        ...
        mensaje = (
            f"No se encontró el archivo 'query_cc_join.sql' en {carpeta_queries}\n"
        )
...
def cargar_parquets(
    ruta_gold: Path, ruta_cc_actual: Path, ruta_cc_old: Path
    ...
    print(
        f"    ✓ {len(df_cc_actual):,} códigos únicos | {len(df_cc_actual.columns)} columnas"
    )
    ...
    print(
        f"    ✓ {len(df_cc_old):,} códigos únicos | {len(df_cc_old.columns)} columnas"
    )

    return df_examenes, df_cc_actual, df_cc_old


def ejecutar_join_sql(
...
def analizar_resultados(df: pl.DataFrame) -> dict:
    ...
    stats_dict = {
        "total": total,
        "ok": stats.filter(pl.col("status_match") == "OK")["cantidad"].sum()
        if "OK" in stats["status_match"]
        else 0,
        "sin_codigo": stats.filter(pl.col("status_match") == "SIN_CODIGO")[
            "cantidad"
        ].sum()
        if "SIN_CODIGO" in stats["status_match"]
        else 0,
        "no_encontrado": stats.filter(pl.col("status_match") == "CODIGO_NO_ENCONTRADO")[
            "cantidad"
        ].sum()
        if "CODIGO_NO_ENCONTRADO" in stats["status_match"]
        else 0,
    }
...
def guardar_resultados(
    ...
    if ruta_excel_actual is not None:
        print("    - Guardando Excel...", end="", flush=True)
        print(" ✓")
    else:
...
def main():
    ...
    print("=" * 80)
    print(
        " ENRIQUECIMIENTO DE EXÁMENES DE RETIRO CON CENTROS DE COSTO ".center(80, "=")
    )
    print("=" * 80)
    ...
    try:
        ...
        (
            ...
            ruta_parquet_historico,
            ruta_excel_historico,
        ) = guardar_resultados(df_resultado, ruta_gold, stats)
        ...
        print(
            f"  - Enriquecidos exitosamente:  {stats['ok']:,} ({stats['ok'] / stats['total'] * 100:.1f}%)"
        )
        print(
            f"  - Sin código CC:              {stats['sin_codigo']:,} ({stats['sin_codigo'] / stats['total'] * 100:.1f}%)"
        )
        print(
            f"  - Código no encontrado:       {stats['no_encontrado']:,} ({stats['no_encontrado'] / stats['total'] * 100:.1f}%)"
        )
        ...
        print("\n💡 Notas:")
        print(
            "  - Archivos actuales: se sobreescriben en cada ejecución (rutas estables para Power BI)"
        )
