...
def transformar_silver_a_gold(df_silver: pl.DataFrame, esquema: dict) -> pl.DataFrame:
    ...
    columnas_base = [
        col for col, config in schema_def.items() if "derivado_de" not in config
    ]
    ...
    for col_nombre in df_gold.columns:
        ...
        if tipo_str == "date":
            ...
            expresion = (
                pl.col(col_nombre).cast(tipo_polars, strict=False).alias(col_nombre)
            )
    ...
    if "FECHA DE CESE" in df_gold.columns:
        # AÑO
        df_gold = df_gold.with_columns(pl.col("FECHA DE CESE").dt.year().alias("AÑO"))
        print("  ✓ Columna derivada generada: AÑO")

        # MES
        df_gold = df_gold.with_columns(pl.col("FECHA DE CESE").dt.month().alias("MES"))
        print("  ✓ Columna derivada generada: MES")

        df_gold = add_month_name_column(df_gold, default_invalid=None)
        print("  ✓ Columna derivada generada: NOMBRE_MES")
    ...
    if "filtros" in esquema:
        for filtro in esquema["filtros"]:
            ...
            if col_filtro not in df_gold.columns:
                print(
                    f"  ⚠️  No se puede aplicar filtro: columna {col_filtro} no existe"
                )
                continue
...
def guardar_resultados(
    ...
    ruta_excel_actual = maybe_write_excel(
        ruta_excel_actual,
        export_excel,
        lambda path: df_export.write_excel(path),
    )
...
def main():
    ...
    if not ruta_esquema:
        ...
        ruta_esquema = filedialog.askopenfilename(
            title="Seleccionar esquema (YAML)", filetypes=structured_filetypes()
        )
