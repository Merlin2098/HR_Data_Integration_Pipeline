...
class ExamenRetiroWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.label_bronze_status.setStyleSheet(
        ...
        self.label_cc_actual_status.setStyleSheet(
            "color: red; font-size: 16px; font-weight: bold;"
        )
        ...
        self.label_cc_old_status.setStyleSheet(
            "color: red; font-size: 16px; font-weight: bold;"
        )
    ...
    def _select_bronze(self):
        ...
        if archivo.suffix.lower() not in [".xlsx", ".xlsm", ".xls"]:
            self._log("⚠️ El archivo debe ser Excel (.xlsx, .xlsm o .xls)")
            return
    ...
    def _select_cc_actual(self):
        ...
        if archivo.suffix.lower() != ".parquet":
            self._log("⚠️ El archivo debe ser Parquet (.parquet)")
    ...
    def _check_all_files_selected(self):
        ...
        all_selected = (
            self.archivo_bronze is not None
            and self.archivo_cc_actual is not None
            and self.archivo_cc_old is not None
        )

        self.btn_process.setEnabled(all_selected)
        self.btn_clear.setEnabled(
            self.archivo_bronze is not None
            or self.archivo_cc_actual is not None
            or self.archivo_cc_old is not None
        )
