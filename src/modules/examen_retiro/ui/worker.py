...
from src.utils.logger_qt import UILogger  # noqa: E402
from src.utils.validate_source import (  # noqa: E402
    SourceValidationError,
    validate_all_sources_for_etl,
)
from src.utils.structured_config import (  # noqa: E402
...
class ExamenRetiroWorker(QThread):
            ...
            self.finished.emit(
                ...
                f"❌ Error crítico: {str(e)}",
                {
                    ...
                    "error_details": self._build_error_details(
                        stage_name="Ejecución ETL Exámenes de Retiro", error=e
                    ),
    ...
    def execute_etl(self) -> Dict:
        ...
        try:
            ...
            preflight = validate_all_sources_for_etl(
                "examen_retiro", self.archivo_bronze
            )
            ...
            try:
                ...
                # LAZY LOADING: step1
                extraer_bronze_examenes_retiro = (
                    self.loader.step1.extraer_bronze_examenes_retiro
                )
                limpiar_silver_examenes_retiro = (
                    self.loader.step1.limpiar_silver_examenes_retiro
                )
                ...
                self.progress_updated.emit(
                    40, f"✓ Silver generado: {len(df_silver):,} registros"
                )
            ...
            try:
                esquema_path = resolve_structured_path(
                    "assets/esquemas/esquema_examen_retiro"
                )
                ...
                if not esquema_path.exists():
                    ...
                    # Cargar esquema YAML
                    esquema = load_structured_data(
                        esquema_path, prefer_resource_path=False
                    )

                    self.logger.info(
                        f"✓ Esquema cargado: v{esquema['metadata']['version']}"
                    )
                    self.logger.info(
                        f"  • Columnas esperadas: {len(esquema['schema'])}"
                    )
                    ...
                    self.progress_updated.emit(
                        55, "⚙️ Cargando módulo de transformación..."
                    )

                    # LAZY LOADING: step2
                    transformar_silver_a_gold = (
                        self.loader.step2.transformar_silver_a_gold
                    )
                    ...
                    self.progress_updated.emit(
                        60, "🔄 Aplicando transformaciones Gold..."
                    )
                    ...
                    (
                        ruta_parquet_gold_actual,
                        ruta_excel_gold_actual,
                        ruta_parquet_gold_historico,
                        ruta_excel_gold_historico,
                    ) = guardar_resultados_step2(
                        df_gold,
                        carpeta_silver,
                    ...
                    self.progress_updated.emit(
                        70, f"✓ Gold generado: {len(df_gold):,} registros"
                    )

            except ImportError as e:
                self.logger.warning(f"⚠️ Step 2 no disponible: {e}")
                self.progress_updated.emit(
                    100, "✓ Silver completado (Step 2 no disponible)"
                )
            ...
            try:
                ...
                if "step2" not in resultado or "parquet_gold" not in resultado["step2"]:
                    self.logger.warning(
                        "⚠️ No se puede ejecutar Step 3: falta output de Step 2"
                    )
                    ...
                    try:
                        ...
                        ruta_query = get_resource_path(
                            "assets/queries/query_cc_join.sql"
                        )

                        if not ruta_query.exists():
                            raise FileNotFoundError(
                                f"No se encontró query_cc_join.sql en {ruta_query.parent}"
                            )
                        ...
                        self.logger.info(
                            f"✓ Query SQL cargada ({len(query)} caracteres)"
                        )
                    ...
                    df_examenes, df_cc_actual, df_cc_old = cargar_parquets(
                        ruta_gold, self.archivo_cc_actual, self.archivo_cc_old
                    )

                    self.logger.info("✓ Parquets cargados exitosamente")

                    self.progress_updated.emit(85, "🔄 Ejecutando JOIN con DuckDB...")

                    # Ejecutar JOIN
                    df_resultado = ejecutar_join_sql(
                        df_examenes, df_cc_actual, df_cc_old, query
                    )

                    self.logger.info(
                        f"✓ JOIN ejecutado: {len(df_resultado):,} registros"
                    )
                    ...
                    # Guardar resultados
                    (
                        ruta_parquet_enriq_actual,
                        ruta_excel_enriq_actual,
                        ruta_parquet_enriq_historico,
                        ruta_excel_enriq_historico,
                    ) = guardar_resultados_step3(
                        df_resultado,
                        ruta_gold,
                        stats,
                        export_excel=self.export_excel_gold,
                    ...
                    self.logger.info(
                        f"  • Registros enriquecidos: {len(df_resultado):,}"
                    )
                    self.logger.info(
                        f"  • OK: {stats['ok']:,} ({stats['ok'] / stats['total'] * 100:.1f}%)"
                    )
                    ...
                    self.progress_updated.emit(
                        100, f"✓ Gold Enriquecido: {len(df_resultado):,} registros"
                    )

            except ImportError as e:
                self.logger.warning(f"⚠️ Step 3 no disponible: {e}")
                self.progress_updated.emit(
                    100, "✓ Gold completado (Step 3 no disponible)"
                )
            ...
            return {
                ...
                "error": str(e),
                "error_details": self._build_error_details(
                    stage_name="ETL completo Exámenes de Retiro", error=e
                ),
                "timers": self.timers,
            }
        ...
        if isinstance(error_details, dict) and error_details:
            ...
            if stage_name:
                if stage_index and total_stages:
                    self.logger.error(
                        f"Etapa fallida: {stage_index}/{total_stages} - {stage_name}"
                    )
                else:
