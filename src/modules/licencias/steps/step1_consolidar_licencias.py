...
def leer_hoja_excel(
    ruta_archivo: Path, nombre_hoja: str, tipo_licencia: str, esquema: dict
) -> pl.DataFrame | None:
    """
    ...
    try:
        ...
        for fila in ws.iter_rows(min_row=3, values_only=True):
            ...
            fila_filtrada = [
                fila[idx] if idx < len(fila) else None for idx in indices_validos
            ]
        ...
        if "PERIODO" in df.columns:
            df = df.with_columns(pl.col("PERIODO").cast(pl.Utf8).alias("PERIODO"))
        ...
        df = df.with_columns(pl.lit(tipo_licencia).alias("TIPO_LICENCIA"))
...
def main():
    ...
    try:
        ...
        df_con_goce = leer_hoja_excel(
            archivo_bronze, "LICENCIA CON GOCE", "CON_GOCE", esquema
        )

        # Hoja 2: SIN GOCE
        df_sin_goce = leer_hoja_excel(
            archivo_bronze, "LICENCIA SIN GOCE", "SIN_GOCE", esquema
        )
        ...
        columnas_disponibles = [
            col for col in columnas_esquema if col in df_consolidado.columns
        ]
        ...
        consolidar_y_exportar(
            df_consolidado, carpeta_trabajo, registros_con_goce, registros_sin_goce
        )
        ...
        print("\n📂 Archivos generados:")
        print("  - licencias_consolidadas.parquet")
...
def procesar_sin_gui(ruta_archivo: Path, carpeta_salida: Path) -> dict:
    ...
    try:
        ...
        df_con_goce = leer_hoja_excel(
            ruta_archivo, "LICENCIA CON GOCE", "CON_GOCE", esquema
        )

        df_sin_goce = leer_hoja_excel(
            ruta_archivo, "LICENCIA SIN GOCE", "SIN_GOCE", esquema
        )
