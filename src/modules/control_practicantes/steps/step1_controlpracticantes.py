...
def validar_esquema(df: pl.DataFrame, esquema: dict) -> tuple[bool, list[str]]:
    ...
    for col_nombre, tipo_esperado in tipos_esperados.items():
        ...
        if (
            ...
            and "Utf8" not in tipo_actual
            and "String" not in tipo_actual
        ):
            errores.append(
...
def leer_hoja_practicantes(
    ruta_archivo: Path, esquema: dict, raise_on_error: bool = False
) -> pl.DataFrame | None:
    """
    ...
    try:
        ...
        if not headers:
            wb.close()
            abortar(
                "No se encontraron headers válidos en la fila 4 de la hoja 'Practicantes'"
            )
            return None
        ...
        if columnas_faltantes:
            ...
            abortar(
                f"Columnas requeridas no encontradas en 'Practicantes': {columnas_str}"
            )
            return None

        print("   ✓ Todas las columnas objetivo encontradas")
        ...
        if "FECHA ING" in df.columns:
            print("   → Convirtiendo FECHA ING a formato estándar...")

            df = df.with_columns(
                [
                    pl.col("FECHA ING")
                    .map_elements(
                        lambda x: convertir_fecha_excel(x), return_dtype=pl.Utf8
                    )
                    .alias("FECHA ING")
                ]
            )
            ...
            try:
                ...
                print("   ✓ FECHA ING convertida a formato Date")
            except Exception as e:
        ...
        if "F. RENOVACION" in df.columns:
            print("   → Convirtiendo F. RENOVACION a formato estándar...")
            ...
            try:
                ...
                print("   ✓ F. RENOVACION convertida a formato Date")
            except Exception as e:
        ...
        traceback.print_exc()
        if raise_on_error:
            raise RuntimeError(f"Error al leer la hoja 'Practicantes': {e}") from e
        return None
...
def exportar_silver(df: pl.DataFrame, carpeta_silver: Path):
    """
...
def main():
    ...
    print("   ✓ Carpetas creadas: silver/ gold/")

    # 3. Cargar esquema
    print("\n[1/2] Cargando esquema de validación...")

    try:
        ruta_esquema = resolve_structured_path(
            "assets/esquemas/esquema_control_practicantes"
        )
    ...
    try:
        ...
        print("  - control_practicantes_silver.parquet")
