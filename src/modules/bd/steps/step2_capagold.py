...
def validar_y_filtrar_columnas(
    df: pl.DataFrame, esquema: dict
    ...
    print(
        f"  ✓ DataFrame filtrado: {df_filtered.height:,} filas × {df_filtered.width} columnas"
    )
...
def dividir_por_modalidad(df: pl.DataFrame) -> tuple[pl.DataFrame, pl.DataFrame]:
    ...
    print(
        f"    ✓ Practicantes (TERMINO DE CONVENIO): {df_practicantes.height:,} registros"
    )
...
def guardar_resultados(
    ...
    if df_empleados.height > 0:
        ...
        print("    - Guardando actual (parquet)...", end="", flush=True)
        ...
        if ruta_emp_excel_actual is not None:
            print("    - Guardando actual (Excel)...", end="", flush=True)
            print(" ✓")
        else:
            print("    - Excel actual omitido (exportación opcional desactivada)")

        # Histórico
        ruta_emp_parquet_hist = (
            carpeta_historico / f"bd_empleados_gold_{timestamp}.parquet"
        )
        ruta_emp_excel_hist = carpeta_historico / f"bd_empleados_gold_{timestamp}.xlsx"

        print("    - Guardando histórico (parquet)...", end="", flush=True)
        ...
        if ruta_emp_excel_hist is not None:
            print("    - Guardando histórico (Excel)...", end="", flush=True)
            print(" ✓")
        else:
...
def main():
    ...
    try:
        ...
        print(
            f"  ✓ Silver cargado: {df_silver.height:,} filas × {df_silver.width} columnas"
        )
