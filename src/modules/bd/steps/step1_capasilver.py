...
def extraer_datos_excel(
    file_path: Path, header_row: int = 10, data_start_row: int = 11
    ...
    for row in ws.iter_rows(
        min_row=data_start_row, max_col=len(headers), values_only=True
    ):
        # Verificar si la columna NUMERO DE DOC tiene valor
        if (
            row[numero_doc_col_idx - 1] is None
            or str(row[numero_doc_col_idx - 1]).strip() == ""
        ):
            break
...
def crear_dataframe_polars(headers: list[str], data_rows: list[list]) -> pl.DataFrame:
    ...
    for key in data_dict:
        data_dict[key] = [
            None if v in ["None", "nan", "NaT"] else v for v in data_dict[key]
        ]
...
def main():
    ...
    try:
        ...
        headers, data_rows = extraer_datos_excel(
            archivo_bronze, header_row=10, data_start_row=11
        )
