...
def seleccionar_archivo(titulo: str) -> Path | None:
    ...
    archivo = filedialog.askopenfilename(
        title=titulo, filetypes=[("Parquet files", "*.parquet"), ("All files", "*.*")]
    )
...
def generar_resumen_flags(df: pl.DataFrame, columnas_originales: list) -> dict:
    ...
    if "Modalidad de Contrato" in df.columns:
        modalidades = (
            df.group_by("Modalidad de Contrato")
...
def main():
    ...
    try:
        ...
        ruta_p_act, ruta_e_act, ruta_p_hist, ruta_e_hist = guardar_resultados(
            df_con_flags, carpeta_gold
        )
    except Exception as e:
    ...
    for flag_name, flag_data in stats["flags_generadas"].items():
        print(
            f"  • {flag_name}: {flag_data['count']:,} empleados ({flag_data['porcentaje']}%)"
        )
    ...
    if stats["modalidades"]:
        ...
        for modalidad, modalidad_data in stats["modalidades"].items():
            print(
                f"  • {modalidad}: {modalidad_data['count']:,} ({modalidad_data['porcentaje']}%)"
            )
