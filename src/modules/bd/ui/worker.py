...
from src.utils.bd_document_date import (  # noqa: E402
    append_bd_document_date_column,
    extract_bd_document_date,
)
from src.utils.gold_export import maybe_write_excel  # noqa: E402
from src.utils.validate_source import validate_all_sources_for_etl  # noqa: E402
...
class BDWorker(BaseETLWorker):
    ...
    def __init__(self, archivos, output_dir, export_excel_gold: bool = False):
        ...
        # Timers
        self.timers = {"total": 0, "step1": 0, "step1.5": 0, "step2": 0, "step3": 0}
    ...
    def execute_etl(self) -> Dict:
        ...
        try:
            ...
            return self.build_error_result(
                stage_name=self.current_stage, error=str(e), timers=self.timers
            )
    ...
    def _ejecutar_etl_completo(self, tiempo_inicio_total) -> Dict:
        ...
        resultado["step1.5"] = self._step1_5_extraer_centros_costo(
            resultado["step1"]["parquet"]
        )
        ...
        try:
            resultado["step3"] = self._step3_aplicar_flags_automatico(
                resultado["step1.5"]["parquet_actual"]
            )
        ...
        if "error" not in resultado["step3"]:
            mensaje_base += (
                f"  • Flags aplicados: {resultado['step3']['registros']:,} registros\n"
            )
        else:
            mensaje_base += "  ⚠️  Flags: Error (ver log)\n"
    ...
    def _step1_bronze_to_silver(self, archivo_excel):
        ...
        self.logger.info(
            f"  ✓ FECHA_DOCUMENTO detectada desde filename: {fecha_documento}"
        )
        ...
        self.logger.info("-" * 70)
        self.logger.info("✓ Step 1 completado")
    ...
    def _crear_dataframe_polars(self, headers, data_rows, pl):
        ...
        self.logger.info(
            f"  ✓ DataFrame creado: {df.shape[0]:,} filas × {df.shape[1]} columnas"
        )
    ...
    def _step1_5_extraer_centros_costo(self, ruta_silver):
        ...
        self.logger.info(
            f"  ✓ Centros de costo únicos: {df_cc[columna_dedupe].n_unique()}"
        )
        ...
        self.logger.info("-" * 70)
        self.logger.info("✓ Step 1.5 completado")
        self.logger.info(f"  • Registros: {len(df_cc):,}")
        self.logger.info(f"  • Archivo actual: {ruta_actual.name}")
        self.logger.info(
            f"  ⏱️  Duración: {self.logger.format_duration(self.timers['step1.5'])}"
        )
        self.logger.info("-" * 70)
    ...
    def _step2_silver_to_gold(self, ruta_silver):
        ...
        df_silver = pl.read_parquet(ruta_silver)
        self.logger.info(
            f"  ✓ Silver: {len(df_silver):,} registros × {len(df_silver.columns)} columnas"
        )
        ...
        self.logger.info("-" * 70)
        self.logger.info("✓ Step 2 completado")
    ...
    def _convertir_fechas_gold(self, df, esquema, pl):
        ...
        date_columns = [
            col["name"] for col in esquema["columns"] if col["type"] == "date"
        ]
    ...
    def _guardar_gold(self, df_empleados, df_practicantes, carpeta_trabajo):
        ...
        if df_practicantes.height > 0:
            ...
            # Histórico
            ruta_prac_hist = (
                carpeta_historico / f"bd_practicantes_gold_{timestamp}.parquet"
            )
    ...
    def _step3_aplicar_flags_automatico(self, ruta_cc_actual):
        ...
        if not self.duckdb:
            ...
            try:
                ...
                raise ImportError(
                    "DuckDB no está instalado. Instala con: pip install duckdb"
                )
        ...
        self.logger.info("\n📁 Ubicación:")
        self.logger.info("  • Actuales: gold/")
        self.logger.info("  • Históricos: gold/historico/")

        self.timers["step3"] = time.time() - tiempo_inicio

        self.logger.info("-" * 70)
        self.logger.info("✓ Step 3 completado")
