...
def agregar_nombre_mes(df):
    ...
    print("✓ Columna 'NOMBRE_MES' agregada después de 'MES'")
...
def validar_constraints(df, schema):
    ...
    if pk_cols:
        ...
        if len(duplicados) > 0:
            errores.append(
                f"❌ Se encontraron {len(duplicados)} registros duplicados en primary key: {pk_cols}"
            )
        else:
    ...
    for col_name, col_spec in schema["schema"].items():
        ...
        if not col_spec.get("nullable", True):
            ...
            if nulls > 0:
                errores.append(
                    f"❌ Columna '{col_name}' tiene {nulls} valores nulos (no permitidos)"
                )
    ...
    for col_name, col_spec in schema["schema"].items():
        ...
        if "allowed_values" in col_spec:
            ...
            if len(valores_invalidos) > 0:
                ...
                warnings.append(
                    f"⚠️  Columna '{col_name}' tiene {len(valores_invalidos)} valores fuera del rango permitido: {valores_unicos}"
                )
    ...
    for col_name, col_spec in schema["schema"].items():
        ...
        if "min_value" in col_spec:
            ...
            if fuera_rango > 0:
                warnings.append(
                    f"⚠️  Columna '{col_name}' tiene {fuera_rango} valores menores al mínimo permitido ({col_spec['min_value']})"
                )
...
def generar_excel_visualizacion(df, ruta_salida):
    ...
    # Guardar
    wb.save(ruta_salida)
    print("    ✓ Excel generado con formato")
...
def guardar_resultados(df_gold, carpeta_silver, export_excel: bool = False):
    ...
    try:
        ...
        size_mb = (
            ruta_parquet_gold.stat().st_size / (1024 * 1024)
            if ruta_parquet_gold.exists()
            else 0
        )
    ...
    try:
        ...
        if ruta_excel_gold is not None:
            size_mb = (
                ruta_excel_gold.stat().st_size / (1024 * 1024)
                if ruta_excel_gold.exists()
                else 0
            )
...
def exportar_a_gold(
    ...
    # ==================== DEBUG ====================
    print("🔍 DEBUG - Iniciando exportar_a_gold()")
    ...
    try:
        ...
        print("   ✓ Gold guardado exitosamente")
        print("     • Estructura: gold/")
        print("       └── actual/    (archivos actuales)")
...
def main():
    ...
    if carpeta_esquemas is None:
        ...
        if not carpeta_esquemas.exists():
            print("⚠️  No se encontró la carpeta 'esquemas'")
            ...
            print()
            print(
                "❌ Por favor, coloca los archivos de esquema (.yaml/.yml) en esta carpeta y ejecuta nuevamente."
            )
            return
    ...
    if not esquemas_disponibles:
        ...
        print(
            "   Por favor, coloca los archivos de esquemas (.yaml/.yml) en esta carpeta."
        )
        return
    ...
    print("   gold/")
    print("   └── actual/        (Power BI apunta aquí)")
    print("       ├── Planilla_Metso_Consolidado.parquet")
    if rutas["excel"] is not None:
        print("       └── Planilla_Metso_Consolidado.xlsx")
