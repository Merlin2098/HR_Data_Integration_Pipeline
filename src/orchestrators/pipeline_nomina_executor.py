...
class PipelineNominaExecutor(QObject):
    ...
    def validate_structure(self) -> bool:
        ...
        if not archivo_licencias.exists():
            self._log(
                "ERROR", "No se encontró archivo: licencias/CONTROL DE LICENCIAS.xlsx"
            )
            return False
    ...
    def _prepare_stage_params(
        ...
        if stage_index == 0:  # Stage 1: Nómina Bronze → Silver
            return {"archivos": self.archivos, "carpeta_trabajo": self.output_dir}
            ...
            return {"ruta_archivo": archivo_licencias, "carpeta_salida": carpeta_salida}

        elif stage_index == 2:  # Stage 3: Nómina Silver → Gold
            ruta_parquet_silver = (
                self.output_dir / "silver" / "Planilla_Metso_Consolidado_Silver.parquet"
            )
            ...
            ruta_nomina_gold = (
                ...
                / "gold"
                / "actual"
                / "Planilla_Metso_Consolidado.parquet"
            )
