...
class PipelineControlPracticantesExecutor(QObject):
    ...
    def validate_structure(self) -> bool:
        ...
        try:
            ...
            if "Practicantes" not in wb.sheetnames:
                self._log("ERROR", "El archivo no contiene la pestaña 'Practicantes'")
                self._log("ERROR", f"Pestañas disponibles: {wb.sheetnames}")
                wb.close()
                return False

            wb.close()
            self._log("INFO", "✓ Pestaña 'Practicantes' encontrada")

        except Exception as e:
            self._log("ERROR", f"Error al validar archivo Excel: {e}")
            return False

        self._log("INFO", "✓ Estructura validada correctamente")

        return True

    def execute_stage(
        self, stage_config: Dict, stage_index: int, total_stages: int
    ) -> bool:
        """
        ...
        try:
            ...
            self._log("INFO", "→ Ejecutando stage...")
            ...
            self._log("INFO", "✓ Stage completado exitosamente")
    ...
    def _prepare_stage_params(
        ...
        if stage_index == 0:  # Stage 1: Bronze → Silver
            ...
            return {"ruta_archivo": self.archivo, "carpeta_salida": carpeta_silver}

        elif stage_index == 1:  # Stage 2: Silver → Gold
            # step2_controlpracticantes.procesar_sin_gui(ruta_silver, carpeta_gold)
            ruta_silver = (
                self.output_dir / "silver" / "control_practicantes_silver.parquet"
            )
    ...
    def _validate_outputs(self, outputs_config: list):
        ...
        for output in outputs_config:
            ...
            if output_path.exists():
                ...
                self._log(
                    "INFO",
                    f"  ✓ Output generado: {output_path.name} ({size_mb:.2f} MB)",
                )
    ...
    def execute(self) -> Dict:
        ...
        if not self.load_pipeline():
            return {"success": False, "error": "No se pudo cargar el pipeline YAML"}

        # Validar estructura
        if not self.validate_structure():
            return {"success": False, "error": "Validación de archivo fallida"}
        ...
        for idx, stage_config in enumerate(stages):
            ...
            if not success:
                ...
                if self.pipeline_config["config"].get("stop_on_error", True):
                    ...
                    error_reason = error_details.get(
