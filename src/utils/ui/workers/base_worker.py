...
class BaseETLWorker(QThread):
    ...
    def __init__(
        self, archivos: List[Path], output_dir: Path, export_excel_gold: bool = False
    ):
        super().__init__()
    ...
    def build_error_result(
        ...
        return {
            ...
            else self.resultado.get("timers", {}),
        }
    ...
    def _log_success_summary(self):
        ...
        for phase_name, duration in timers.items():
            if phase_name != "total":
                self.logger.info(
                    f"  • {phase_name}: {self.logger.format_duration(duration)}"
                )
        ...
        if stats["validations_passed"] > 0 or stats["validations_failed"] > 0:
            total_validations = (
                stats["validations_passed"] + stats["validations_failed"]
            )
            self.logger.info(
                f"  • Validaciones: {stats['validations_passed']}/{total_validations} exitosas"
            )
    ...
    def _log_error_summary(self):
        ...
        self.logger.info("")
        self.logger.info(
            f"⏱️  Tiempo transcurrido: {self.logger.format_duration(total_duration)}"
        )
